...
function DeliveriesByYou() {
  ...
  const approvedCount = (counts?.approved ?? counts?.completed) ?? fallbackCounts.approved;
  const rejectedCount = (counts?.rejected ?? counts?.declined) ?? fallbackCounts.rejected;
  ...
  const totalPages = showAll ? 1 : Math.ceil(safeFilteredItems.length / (ITEMS_PER_PAGE || 10));
                    ...
                    <div className="col-action" style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
                      <ActionPopover
                        ...
                        onTriggerClick={() => {
                          ...
                          navigate(rolePath(`deliveries/deliverable-details/${item.id}`), { state: { from: "deliveries-by-you", subtaskIds, page, returnUrl } });
                        }}
                      >
                        {(() => {
