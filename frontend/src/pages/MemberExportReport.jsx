...
function MemberExportReport({ isOpen, onClose, userData }) {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.text("USER PERFORMANCE REPORT", PW / 2, 8, { align: "center" });
      doc.setFontSize(5.5); doc.setFont("helvetica", "normal");
      doc.setTextColor(148, 163, 184);
      const infoRX = PW - M;
      doc.text(`Generated On:  ${genDate}`, infoRX, 5, { align: "right" });
      doc.text(`Generated By:  ${genBy}`, infoRX, 9, { align: "right" });
      doc.text(`Report Type:  User Performance Report`, infoRX, 13, { align: "right" });
      y = 18;
      ...
      doc.text("Report Type:  User Performance Report", PW - M - 50, fY + 4);
      doc.text("Page 1 of 1", PW - M, fY + 7.5, { align: "right" });
      doc.save(`User-Performance-Report-${(user.name || "user").replace(/\s+/g, "-")}.pdf`);
    } catch (err) {
            ...
            <div style={{ padding: "0" }}>
              ...
              <div style={{ background: "#0f172a", padding: "14px 28px", display: "flex", alignItems: "center" }}>
                ...
                <div style={{ flex: 1, textAlign: "center" }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>USER PERFORMANCE REPORT</span>
                </div>
                <div style={{ textAlign: "right", fontSize: 9, color: "#94a3b8", lineHeight: 1.8 }}>
                  <div>Generated On:  {genDate}</div>
                  <div>Generated By:  {genBy}</div>
                  <div>Report Type:  User Performance Report</div>
                </div>
              </div>
              ...
              <div style={{ borderTop: "1px solid #e5e7eb", margin: "0 28px", padding: "8px 0", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 9, color: "#9ca3af" }}>
                ...
                <div>Report Type:  User Performance Report | Page 1 of 1</div>
              </div>
