...
function Reports() {
  ...
  const statusItems = useMemo(() => {
    return [
      { label: t("Completed", { defaultValue: "Completed" }), count: summary?.approved || 0, color: "var(--color-green-text)" },
      { label: t("Pending", { defaultValue: "Pending" }), count: summary?.pending || 0, color: "var(--color-orange-text)" },
      { label: t("In Review", { defaultValue: "In Review" }), count: summary?.in_review || 0, color: "var(--color-blue-text)" },
      { label: t("Overdue", { defaultValue: "Overdue" }), count: summary?.overdue || 0, color: "var(--color-danger)" },
    ];
  }, [summary?.approved, summary?.pending, summary?.in_review, summary?.overdue, t]);

  // Priority breakdown for bar chart — API returns flat summary fields
  const priorityItems = useMemo(() => {
    return [
      { label: t("High", { defaultValue: "High" }), count: summary?.high_priority || 0, color: "var(--color-danger)" },
      { label: t("Medium", { defaultValue: "Medium" }), count: summary?.medium_priority || 0, color: "var(--color-orange-text)" },
      { label: t("Low", { defaultValue: "Low" }), count: summary?.low_priority || 0, color: "var(--color-green-text)" },
    ];
  }, [summary?.high_priority, summary?.medium_priority, summary?.low_priority, t]);
  ...
  const breadcrumbs = [
    { label: t("Reports", { defaultValue: "Reports" }) },
  ];
    ...
    <DashboardLayout>
      ...
      <div className="reports-page">
        ...
        <div className="reports-header">
          <div>
            <h1>{isTeamMembersView ? t("Team Members Performance", { defaultValue: "Team Members Performance" }) : t("Performance Report", { defaultValue: "Performance Report" })}</h1>
            <p>{isTeamMembersView ? t("Track progress and performance of your team members", { defaultValue: "Track progress and performance of your team members" }) : t("Track progress, tasks, and performance across your team", { defaultValue: "Track progress, tasks, and performance across your team" })}</p>
          </div>
          <div className="reports-header-actions" style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            ...
            {isAdminOrManager && (
              ...
              >
                {t("Export Report", { defaultValue: "Export Report" })}
                <span>↓</span>
              </button>
            )}
          </div>
        ...
        {isAdminOrManager && teams.length > 0 && (
          <div className="teams-carousel-section">
            <div className="teams-carousel-header">
              <h3>{t("Teams Overview", { defaultValue: "Teams Overview" })}</h3>
              <div className="reports-section-search">
                <IoSearchOutline size={16} />
                <input
                  type="text"
                  placeholder={t("Search by team name or leader...", { defaultValue: "Search by team name or leader..." })}
                  value={teamSearch}
              ...
              <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                ...
                >
                  {t("Export Team Report", { defaultValue: "Export Team Report" })} <span>↓</span>
                </button>
                <button
                  onClick={() => navigate(rolePath("manage-team"))}
                  className="teams-view-all-btn"
                >
                  {t("View All Teams", { defaultValue: "View All Teams" })}
                </button>
              </div>
            </div>
            {isTeamsLoading ? (
              <div style={{ padding: "24px", textAlign: "center", color: "var(--text-muted)" }}>{t("Loading...", { defaultValue: "Loading..." })}</div>
            ) : (
              <>
                      ...
                      >
                        <div className="team-card-top">
                          ...
                          <div className="team-card-info">
                            ...
                            <p className="team-card-leader">
                              {team.leader ? t("Lead: {{name}}", { name: team.leader.name, defaultValue: `Lead: ${team.leader.name}` }) : t("No leader", { defaultValue: "No leader" })}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="team-card-stats">
                          <div className="team-card-stat">
                            ...
                            <span className="team-card-stat-label">{t("Members", { defaultValue: "Members" })}</span>
                          </div>
                          <div className="team-card-stat">
                            <span className="team-card-stat-value" style={{ color: "var(--color-primary-light)" }}>{team.assigned}</span>
                            <span className="team-card-stat-label">{t("Tasks", { defaultValue: "Tasks" })}</span>
                          </div>
                          <div className="team-card-stat">
                            <span className="team-card-stat-value" style={{ color: "var(--color-success)" }}>{team.completed}</span>
                            <span className="team-card-stat-label">{t("Done", { defaultValue: "Done" })}</span>
                          </div>
                          <div className="team-card-stat">
                            <span className="team-card-stat-value" style={{ color: "var(--color-danger)" }}>{team.overdue}</span>
                            <span className="team-card-stat-label">{t("Overdue", { defaultValue: "Overdue" })}</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          onClick={() => navigate(rolePath(`reports/team-members/${team.id}`))}
                        >
                          {t("View Members", { defaultValue: "View Members" })}
        ...
        {showUserTable && (
          <>
            <div className="reports-table-section-header">
              <h3>{t("User Performance", { defaultValue: "User Performance" })}</h3>
              <div className="reports-section-search">
                <IoSearchOutline size={16} />
                <input
                  type="text"
                  placeholder={t("Search by user name or role...", { defaultValue: "Search by user name or role..." })}
                  value={userSearch}
            ...
            <div className="reports-table-wrapper">
              <div className="reports-table">
                <div className="table-header">
                  <span className="th-member">{t("User", { defaultValue: "User" })}</span>
                  <span className="th-stat">{t("Assigned", { defaultValue: "Assigned" })}</span>
                  <span className="th-stat">{t("Approved", { defaultValue: "Approved" })}</span>
                  <span className="th-stat">{t("Pending", { defaultValue: "Pending" })}</span>
                  <span className="th-tasks">{t("Overdue", { defaultValue: "Overdue" })}</span>
                  <span className="th-action">{t("Action", { defaultValue: "Action" })}</span>
                </div>

                {isTableLoading ? (
                  <div style={{ padding: "24px", textAlign: "center", color: "var(--text-muted)" }}>{t("Loading...", { defaultValue: "Loading..." })}</div>
                ) : (paginatedUsers || []).length === 0 ? (
                  <div style={{ padding: "24px", textAlign: "center", color: "var(--text-muted)" }}>{t("No data available", { defaultValue: "No data available" })}</div>
                ) : (
                    ...
                    <div key={member.id} className="table-row">
                      <div className="table-member">
                        ...
                        <div className="member-info">
                          ...
                          <div className="member-role">{ROLE_LABEL[member.role] ? t(ROLE_LABEL[member.role], { defaultValue: ROLE_LABEL[member.role] }) : member.role}</div>
                        </div>
                      </div>
                  ...
                  <div key={member.id} className="report-member-card">
                    ...
                    <div className="report-member-stats">
                      <div className="report-member-stat">
                        <span className="report-member-stat-label">{t("Assigned", { defaultValue: "Assigned" })}</span>
                        <span className="stat-badge assigned">{member.assigned}</span>
                      </div>
                      <div className="report-member-stat">
                        <span className="report-member-stat-label">{t("Approved", { defaultValue: "Approved" })}</span>
                        <span className="stat-badge completed">{member.completed}</span>
                      </div>
                      <div className="report-member-stat">
                        <span className="report-member-stat-label">{t("Pending", { defaultValue: "Pending" })}</span>
                        <span className="stat-badge pending">{member.pending}</span>
                      </div>
                      <div className="report-member-stat">
                        <span className="report-member-stat-label">{t("Overdue", { defaultValue: "Overdue" })}</span>
                        <span className="stat-badge overdue">{member.overdue}</span>
                      </div>
                    </div>
      ...
      <TeamExportReport isOpen={showTeamExportModal} onClose={() => setShowTeamExportModal(false)} team={{ id: "all", name: t("All Teams", { defaultValue: "All Teams" }) }} />
    </DashboardLayout>
  );
}
