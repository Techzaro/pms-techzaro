        ...
        {loading ? (
          ...
          >
            {(item, idx, dndProps) => {
              ...
              return (
                <div
                  className={`taskby-row ${isBulkMode ? "bulk-mode" : ""} ${isInactive ? "delegation-rejected-row" : ""} ${selectedItems.includes(item.id) ? "row-selected" : ""}`}
                  key={item.sortableId}
                  style={isInactive ? { opacity: 0.88 } : undefined}
                >
                  {isBulkMode && (
