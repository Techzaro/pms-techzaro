        ...
        <div className="rounded-xl p-5 shadow-sm" style={sc.card}>
          ...
          {plan?<div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 pt-3" style={{borderTop:'1px solid var(--border-light)'}}>
              {[{icon:Users,l:'Users',v:plan.max_users>=9999?'Unlimited':plan.max_users},{icon:FolderKanban,l:'Projects',v:plan.max_projects>=9999?'Unlimited':plan.max_projects},{icon:HardDrive,l:'Storage',v:`${plan.max_storage_gb} ${plan.storage_unit || 'GB'}`}].map(x=>
            ...
            <div className="rounded-xl p-5 shadow-sm" style={sc.card}>
              ...
              <div className="mb-3">
                <div className="flex justify-between mb-1">
                  <span className="text-sm font-semibold" style={sc.text}>{fmtBytesToUnit(stSum.summary.total_bytes, stSum.summary.storage_unit || 'GB')} used</span>
                   <span className="text-sm" style={sc.ts}>of {stSum.summary.max_storage_gb} {stSum.summary.storage_unit || 'GB'}</span>
                </div>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <span className="text-xs" style={sc.tm}>{fmtBytesToUnit(stSum.summary.remaining_bytes, stSum.summary.storage_unit || 'GB')} remaining</span>
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  {l:'Total Files',v:stSum.summary.total_files,color:'var(--color-primary)'},
                  {l:'Used Space',v:fmtBytesToUnit(stSum.summary.total_bytes, stSum.summary.storage_unit || 'GB'),color:'var(--color-primary)'},
                   {l:'Storage Limit',v:`${stSum.summary.max_storage_gb} ${stSum.summary.storage_unit || 'GB'}`,color:'var(--color-success)'},
                  {l:'Remaining',v:fmtBytesToUnit(stSum.summary.remaining_bytes, stSum.summary.storage_unit || 'GB'),color:stSum.summary.usage_percent>95?'var(--color-danger)':'var(--color-warning)'},
                ].map(x=>
