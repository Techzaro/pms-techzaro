...
const labelMap = {
  masterDatabase: { key: 'Master Database', defaultLabel: 'Master Database' },
  tenantDatabases: { key: 'Tenant Databases', defaultLabel: 'Tenant Databases' },
  queue: { key: 'Queue System', defaultLabel: 'Queue System' },
  mail: { key: 'Mail Service', defaultLabel: 'Mail Service' },
  cache: { key: 'Cache Layer', defaultLabel: 'Cache Layer' },
  redis: { key: 'Redis', defaultLabel: 'Redis' },
  storage: { key: 'File Storage', defaultLabel: 'File Storage' },
  scheduler: { key: 'Task Scheduler', defaultLabel: 'Task Scheduler' },
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={s.textHeading}>{t('System Health', { defaultValue: 'System Health' })}</h1>
          <p className="text-sm mt-1" style={s.textSecondary}>{t('Real-time platform health status', { defaultValue: 'Real-time platform health status' })}</p>
        </div>
        <button onClick={fetchHealth} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
          <RefreshCw className="w-4 h-4" /> {t('Refresh', { defaultValue: 'Refresh' })}
        </button>
      </div>
      ...
      <div className="p-5 rounded-xl border-2" style={{ background: oc.bg, borderColor: oc.border }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold capitalize" style={s.textHeading}>
              {t('System Status: {{status}}', { status: t(overall, { defaultValue: overall }), defaultValue: `System Status: ${overall}` })}
            </h2>
            <p className="text-sm" style={s.textSecondary}>
              {t('Last checked: {{time}}', { time: new Date().toLocaleString(), defaultValue: `Last checked: ${new Date().toLocaleString()}` })}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div key={key} className="rounded-xl p-5 hover:shadow-md transition-shadow" style={s.card}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold" style={s.textHeading}>
                      {labelInfo ? t(labelInfo.key, { defaultValue: labelInfo.defaultLabel }) : key}
                    </h3>
                    <p className="text-xs capitalize" style={s.textSecondary}>{t(check?.status || 'unknown', { defaultValue: check?.status || 'unknown' })}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2 mt-4">
                {Object.entries(check || {}).filter(([k]) => !['status'].includes(k)).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between text-sm">
                    <span className="capitalize" style={s.textSecondary}>{t(k.replace(/([A-Z])/g, ' $1').trim(), { defaultValue: k.replace(/([A-Z])/g, ' $1').trim() })}</span>
        ...
        {entries.length === 0 && <p className="text-sm col-span-3 text-center py-8" style={s.textMuted}>{t('No health data available', { defaultValue: 'No health data available' })}</p>}
      </div>
    </div>
  );
}
