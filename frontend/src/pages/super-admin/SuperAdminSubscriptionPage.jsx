...
const STATUS_CONFIG = {
  active: { labelKey: 'Active', defaultLabel: 'Active', color: '#16a34a', bg: '#f0fdf4' },
  trial: { labelKey: 'Trial', defaultLabel: 'Trial', color: '#2563eb', bg: '#eff6ff' },
  cancelled: { labelKey: 'Cancelled', defaultLabel: 'Cancelled', color: '#dc2626', bg: '#fef2f2' },
  past_due: { labelKey: 'Past Due', defaultLabel: 'Past Due', color: '#d97706', bg: '#fffbeb' },
  suspended: { labelKey: 'Suspended', defaultLabel: 'Suspended', color: '#dc2626', bg: '#fef2f2' },
};

const MODULE_CATEGORY_CONFIG = {
  core: { labelKey: 'Core Features', defaultLabel: 'Core Features', icon: '\u2605', color: '#4f46e5' },
  standard: { labelKey: 'Standard Features', defaultLabel: 'Standard Features', icon: '\u25c6', color: '#2563eb' },
  enterprise: { labelKey: 'Enterprise Features', defaultLabel: 'Enterprise Features', icon: '\u25c9', color: '#d97706' },
};
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', py: 80 }}>
        ...
        <span style={{ marginLeft: 12, fontSize: 14, color: '#6b7280' }}>{t('Loading subscription...', { defaultValue: 'Loading subscription...' })}</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        <p style={{ color: '#dc2626', fontSize: 14 }}>{t(error, { defaultValue: error })}</p>
        <button onClick={fetchSubscription} style={{ marginTop: 12, fontSize: 14, fontWeight: 600, color: '#4f46e5', background: 'none', border: 'none', cursor: 'pointer' }}>{t('Retry', { defaultValue: 'Retry' })}</button>
      </div>
    );
  }
  ...
  if (!plan) {
    return (
      <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 40, boxShadow: 'var(--shadow-sm)', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-heading)', marginBottom: 8 }}>{t('No Active Plan', { defaultValue: 'No Active Plan' })}</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 20 }}>{t('No subscription plan has been assigned to TechXaro yet.', { defaultValue: 'No subscription plan has been assigned to TechXaro yet.' })}</p>
        <button onClick={openPlanModal} style={{ padding: '10px 24px', borderRadius: 10, background: '#4f46e5', color: '#fff', border: 'none', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
          {t('Assign Plan', { defaultValue: 'Assign Plan' })}
        </button>
    ...
    <div className="space-y-5">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-heading)', margin: 0 }}>
            {t('TechXaro Subscription', { defaultValue: 'TechXaro Subscription' })}
            {organization?.id && (
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>{t("Manage your organization's subscription plan and billing.", { defaultValue: "Manage your organization's subscription plan and billing." })}</p>
        </div>
        <button onClick={openPlanModal} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 10, background: '#4f46e5', color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          {t('Change Plan', { defaultValue: 'Change Plan' })}
        </button>
      </div>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 28, boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              <h2 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-heading)', margin: 0 }}>{plan.name} {t('Plan', { defaultValue: 'Plan' })}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border-light)' }}>
          <InfoItem icon={<Calendar style={{ width: 18, height: 18 }} />} iconBg="#eef2ff" iconColor="#4f46e5" label={t("Started", { defaultValue: "Started" })} value={formatDate(subscription?.starts_at)} />
          {subscription?.ends_at && (
            <InfoItem icon={<Clock style={{ width: 18, height: 18 }} />} iconBg="#fffbeb" iconColor="#d97706" label={t("Renews", { defaultValue: "Renews" })} value={formatDate(subscription?.ends_at)} />
          )}
          <InfoItem icon={<CreditCard style={{ width: 18, height: 18 }} />} iconBg="#f0fdf4" iconColor="#16a34a" label={t("Billing", { defaultValue: "Billing" })} value={isYearly ? t('Yearly', { defaultValue: 'Yearly' }) : t('Monthly', { defaultValue: 'Monthly' })} />
          {usage && (
            <InfoItem icon={<Users style={{ width: 18, height: 18 }} />} iconBg="#f0f9ff" iconColor="#0284c7" label={t("Usage", { defaultValue: "Usage" })} value={t('{{users}} users, {{projects}} projects', { users: usage.users, projects: usage.projects, defaultValue: `${usage.users} users, ${usage.projects} projects` })} />
          )}
        </div>
      </div>
...
<LimitCard icon={<Users style={{ width: 20, height: 20 }} />} label={t("Users", { defaultValue: "Users" })} value={plan.max_users} color="#4f46e5" />
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 24, boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-heading)', margin: 0 }}>{t('Included Modules', { defaultValue: 'Included Modules' })}</h3>
          <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{t('{{count}} active', { count: modules?.total_enabled || 0, defaultValue: `${modules?.total_enabled || 0} active` })}</span>
        </div>
        {allModules.length === 0 ? (
          <p style={{ color: 'var(--text-muted)', fontSize: 14, textAlign: 'center', padding: '20px 0' }}>{t('No modules available for this plan.', { defaultValue: 'No modules available for this plan.' })}</p>
        ) : (
              ...
              return (
                <div key={catKey}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t(catConfig.labelKey, { defaultValue: catConfig.defaultLabel })}</span>
                  </div>
                    ...
                    {catModules.map((mod) => (
                      <div key={mod.slug || mod.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 12, background: mod.is_enabled ? '#f0fdf4' : 'var(--bg-hover)', border: `1px solid ${mod.is_enabled ? '#bbf7d0' : 'var(--border-light)'}` }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 600, color: mod.is_enabled ? 'var(--text-dark)' : 'var(--text-muted)', margin: 0 }}>{t(moduleDisplayName(mod.name, mod.slug), { defaultValue: moduleDisplayName(mod.name, mod.slug) })}</p>
                          {mod.description && <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '2px 0 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t(mod.description, { defaultValue: mod.description })}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 24, boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {history.map((item) => {
              ...
              }[item.event_type] || { labelKey: item.event_type, defaultLabel: item.event_type, icon: Clock, color: '#6b7280', bg: 'var(--bg-hover)' };
                ...
                <div key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 14, borderRadius: 12, background: 'var(--bg-hover)', border: '1px solid var(--border-light)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-dark)' }}>{t(eventConfig.labelKey, { defaultValue: eventConfig.defaultLabel })}</span>
                      <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 600, background: eventConfig.bg, color: eventConfig.color }}>{item.plan?.name || t('Unknown', { defaultValue: 'Unknown' })}</span>
                      {item.previous_plan && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('from {{name}}', { name: item.previous_plan.name, defaultValue: `from ${item.previous_plan.name}` })}</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4, fontSize: 12, color: 'var(--text-muted)' }}>
                      <span>{new Date(item.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}</span>
                      {item.changed_by && <span>{t('by {{name}}', { name: item.changed_by, defaultValue: `by ${item.changed_by}` })}</span>}
                    </div>
                  </div>
                  {item.amount > 0 && (
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-dark)', margin: 0 }}>${item.amount}</p>
                      <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: 0 }}>{t(item.billing_period, { defaultValue: item.billing_period })}</p>
                    </div>
                  )}
                </div>
              );
...
function LimitCard({ icon, label, value, suffix = '', color }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 16, borderRadius: 14, background: 'var(--bg-hover)', border: '1px solid var(--border-light)' }}>
      ...
      <div>
        <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
        <p style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-heading)', margin: '2px 0 0' }}>{isUnlimited ? t('Unlimited', { defaultValue: 'Unlimited' }) : value}{!isUnlimited && suffix ? ` ${suffix}` : ''}</p>
      </div>
    </div>
  );
}
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 20, width: '100%', maxWidth: 520, maxHeight: '80vh', overflow: 'auto', margin: 16 }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, padding: 4, background: 'var(--bg-hover)', borderRadius: 12, marginBottom: 20 }}>
            <button onClick={() => setSelectedBilling('monthly')} style={{ flex: 1, padding: '10px 16px', borderRadius: 10, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', background: selectedBilling === 'monthly' ? '#4f46e5' : 'transparent', color: selectedBilling === 'monthly' ? '#fff' : 'var(--text-secondary)' }}>{t('Monthly', { defaultValue: 'Monthly' })}</button>
            <button onClick={() => setSelectedBilling('yearly')} style={{ flex: 1, padding: '10px 16px', borderRadius: 10, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', background: selectedBilling === 'yearly' ? '#4f46e5' : 'transparent', color: selectedBilling === 'yearly' ? '#fff' : 'var(--text-secondary)' }}>{t('Yearly', { defaultValue: 'Yearly' })}</button>
          </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {isCurrent && <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 12, background: '#dcfce7', color: '#166534' }}>{t('Current', { defaultValue: 'Current' })}</span>}
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>/{selectedBilling === 'yearly' ? t('yr', { defaultValue: 'yr' }) : t('mo', { defaultValue: 'mo' })}</span>
                    </div>
                  </div>
...
<span>{p.max_users >= 9999 ? t('Unlimited', { defaultValue: 'Unlimited' }) : p.max_users} {t('users', { defaultValue: 'users' })}</span>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border-light)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button onClick={onClose} style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid var(--border-light)', background: 'var(--bg-hover)', fontSize: 13, fontWeight: 600, cursor: 'pointer', color: 'var(--text-secondary)' }}>{t('Cancel', { defaultValue: 'Cancel' })}</button>
          <button onClick={onChange} disabled={changing || !selectedPlanId} style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: changing || !selectedPlanId ? '#94a3b8' : '#4f46e5', color: '#fff', fontSize: 13, fontWeight: 600, cursor: changing || !selectedPlanId ? 'not-allowed' : 'pointer' }}>
            {changing ? t('Changing...', { defaultValue: 'Changing...' }) : t('Confirm Change', { defaultValue: 'Confirm Change' })}
          </button>
        </div>
      </div>
    </div>
