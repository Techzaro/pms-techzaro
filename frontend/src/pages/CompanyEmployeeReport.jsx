...
function CompanyEmployeeReport({ isOpen, onClose }) {
  ...
  const periodLabel = dateRange === "custom" && customStart && customEnd
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.text(`Generated By:  Admin`, infoRX, 9, { align: "right" });
      doc.text(`Report Type:  Company Employee Report`, infoRX, 13, { align: "right" });
      y = 18;
            ...
            <div style={{ padding: 0 }}>
              ...
              <div style={{ background: "#0f172a", padding: "14px 28px", display: "flex", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "right", fontSize: 9, color: "#94a3b8", lineHeight: 1.8 }}>
                  ...
                  <div>Report Type:  Company Employee Report</div>
                  <div>Report Period:  {periodLabel}</div>
                </div>
              </div>

              {/* ═══ COMPANY OVERVIEW + SUMMARY CARDS ═══ */}
              <div style={{ margin: "14px 28px", display: "grid", gridTemplateColumns: "200px 1fr", gap: 10 }}>
                {/* Company Overview */}
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                    <thead>
                      <tr style={{ background: "#111827", color: "#fff", fontWeight: 600, fontSize: 9 }}>
                        {["#", "Employee", "Role", "Assigned", "Completed", "Pending", "Overdue", "Completion Rate"].map(h => (
                          <th key={h} style={{ padding: "6px 8px", textAlign: ["#", "Assigned", "Completed", "Pending", "Overdue", "Completion Rate"].includes(h) ? "center" : "left" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {employees.length === 0 ? (
                        ...
                        return (
                          <tr key={e.id} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 ? "#f9fafb" : "#fff" }}>
                            ...
                            <td style={{ padding: "6px 8px", color: "#374151" }}>{ROLE_LABEL[e.role] || e.role}</td>
                            <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 600, color: "#6366f1" }}>{e.assigned ?? 0}</td>
                            <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 600, color: "#22c55e" }}>{e.completed ?? 0}</td>
                            <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 600, color: "#f59e0b" }}>{e.pending ?? 0}</td>
                            <td style={{ padding: "6px 8px", textAlign: "center", fontWeight: 600, color: "#ef4444" }}>{e.overdue ?? 0}</td>
                    ...
                    <tbody>
                      {teams.map((t, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 ? "#f9fafb" : "#fff" }}>
                          <td style={{ padding: "4px 6px", fontWeight: 600, color: "#111827" }}>{t.name}</td>
                          <td style={{ padding: "4px 6px", textAlign: "center", color: "#374151" }}>{t.assigned ?? 0}</td>
                          <td style={{ padding: "4px 6px", textAlign: "center", color: "#22c55e", fontWeight: 600 }}>{t.completed ?? 0}</td>
                          <td style={{ padding: "4px 6px", textAlign: "center", color: "#f59e0b", fontWeight: 600 }}>{t.pending ?? 0}</td>
                          <td style={{ padding: "4px 6px", textAlign: "center", fontWeight: 700, color: (t.completion_rate ?? 0) >= 80 ? "#22c55e" : (t.completion_rate ?? 0) >= 50 ? "#f59e0b" : "#ef4444" }}>
                            {t.completion_rate ?? 0}%
                          </td>
                        </tr>
                      ))}
