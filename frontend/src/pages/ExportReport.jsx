...
function ExportReport({ isOpen, onClose, summary = {}, users = [] }) {
  ...
  const periodLabel = timeRange === "custom" && customStart && customEnd
    ? `${new Date(customStart).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })} - ${new Date(customEnd).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}`
    : ({ all: "All Time", today: "Today", week: "Last 7 Days", month: "Last 30 Days" }[timeRange] || "All Time");
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.setFontSize(5.5); doc.setFont("helvetica", "normal"); doc.setTextColor(148, 163, 184);
      doc.text(`Generated On:  ${genDate}`, PW - M, 5, { align: "right" });
      doc.text(`Generated By:  Admin`, PW - M, 9, { align: "right" });
      doc.text(`Report Type:  Performance Report`, PW - M, 13, { align: "right" });
      y = 18;

      // ── SECTION TITLE ──
      doc.setFontSize(9); doc.setFont("helvetica", "bold");
      doc.setTextColor(17, 24, 39); doc.text("SUMMARY OVERVIEW", M, y + 2);
      y += 8;
        ...
        <div className="er-overlay">
          <div className="er-review-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 820 }}>
            ...
            <div style={{ padding: 0 }}>
              ...
              {/* SUMMARY CARDS */}
              <div style={{ margin: "14px 28px", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                {cardData.map((c) => (
                    ...
                    <thead>
                      <tr style={{ background: "#111827", color: "#fff", fontWeight: 600, fontSize: 9 }}>
                        {["#", "User", "Role", "Assigned", "Completed", "Pending", "Overdue"].map(h => (
                          <th key={h} style={{ padding: "6px 10px", textAlign: h === "#" ? "center" : "left" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {(users || []).length === 0 ? (
                        ...
                        <tr key={u.id} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 ? "#f9fafb" : "#fff" }}>
                          <td style={{ padding: "6px 10px", textAlign: "center", color: "#6b7280" }}>{i + 1}</td>
                          <td style={{ padding: "6px 10px", fontWeight: 600, color: "#111827" }}>{u.name || "-"}</td>
                          <td style={{ padding: "6px 10px", color: "#374151" }}>{ROLE_LABEL[u.role] || u.role || "-"}</td>
                          <td style={{ padding: "6px 10px", fontWeight: 600, color: "#6366f1" }}>{u.assigned ?? 0}</td>
                          <td style={{ padding: "6px 10px", fontWeight: 600, color: "#22c55e" }}>{u.completed ?? 0}</td>
                          <td style={{ padding: "6px 10px", fontWeight: 600, color: "#f59e0b" }}>{u.pending ?? 0}</td>
                          <td style={{ padding: "6px 10px", fontWeight: 600, color: "#ef4444" }}>{u.overdue ?? 0}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* FOOTER */}
              <div style={{ borderTop: "1px solid #e5e7eb", margin: "0 28px", padding: "8px 0", display: "flex", justifyContent: "space-between", fontSize: 9, color: "#9ca3af" }}>
