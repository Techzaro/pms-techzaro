...
function ProjectSubmissionPanel({
  ...
  const latestAttachments = latestSubmission?.attachments || [];
  const fileAttachments = latestAttachments.filter((item) => item.attachment_type === "file");
  const imageAttachments = latestAttachments.filter((item) => item.attachment_type === "image");
  const linkAttachments = latestAttachments.filter((item) => item.attachment_type === "link");
      ...
      {(status === "submitted" || status === "approved" || status === "rejected") && latestSubmission && (
        <div className="td-card td-submission-card">
          ...
          {fileAttachments.length > 0 && (
            <div className="td-submission-item" style={{ marginTop: "12px" }}>
              <span className="td-submission-label">Submitted Files</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginTop: "6px" }}>
                {fileAttachments.map((file) => (
                  <a key={`project-file-${file.id}`} className="td-submission-file-link" href={buildAttachmentUrl(file)} target="_blank" rel="noopener noreferrer">
                    <FileText size={16} />
          ...
          {imageAttachments.length > 0 && (
            <div className="td-submission-item" style={{ marginTop: "12px" }}>
              <span className="td-submission-label">Submitted Images</span>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginTop: "8px" }}>
                {imageAttachments.map((image) => (
                  <a key={`project-image-${image.id}`} href={buildAttachmentUrl(image)} target="_blank" rel="noopener noreferrer">
                    <img
          ...
          {linkAttachments.length > 0 && (
            <div className="td-submission-item" style={{ marginTop: "12px" }}>
              <span className="td-submission-label">Submitted Links</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginTop: "6px" }}>
                {linkAttachments.map((link) => (
                  <a key={`project-link-${link.id}`} className="td-submission-file-link" href={buildAttachmentUrl(link)} target="_blank" rel="noopener noreferrer">
                    <span>{link.url}</span>
                  </a>
                ))}
              </div>
