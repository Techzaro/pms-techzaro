...
export default function ChangePasswordModal({ onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!currentPassword) {
      errs.currentPassword = t("Please enter current password.", { defaultValue: "Please enter current password." });
    }
    if (!confirmPassword) {
      errs.confirmPassword = t("Please confirm your password.", { defaultValue: "Please confirm your password." });
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (msg.toLowerCase().includes("current password") || msg.toLowerCase().includes("old_password") || msg.toLowerCase().includes("incorrect")) {
          if (!resErrs.currentPassword) resErrs.currentPassword = t("Current password is incorrect.", { defaultValue: "Current password is incorrect." });
        }
        if (msg.toLowerCase().includes("confirm") || msg.toLowerCase().includes("match")) {
          if (!resErrs.confirmPassword) resErrs.confirmPassword = t("Password confirmation does not match", { defaultValue: "Password confirmation does not match" });
        }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10010, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)" }} onClick={handleClose}>
      <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: "28px 30px 24px", width: 440, maxWidth: "92vw", boxShadow: "0 25px 60px rgba(0,0,0,0.25)", maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "var(--text-heading)" }}>{t("Change Password")}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>{t("Update your account password.", { defaultValue: "Update your account password." })}</p>
          </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "var(--text-heading)", marginBottom: 6 }}>{t("Current Password", { defaultValue: "Current Password" })}</label>
          <div style={{ position: "relative" }}>
            ...
            <input type={showCurrent ? "text" : "password"} value={currentPassword} onChange={(e) => { setCurrentPassword(e.target.value); setErrors((prev) => ({ ...prev, currentPassword: "" })); }} placeholder={t("Enter current password", { defaultValue: "Enter current password" })} style={{ width: "100%", padding: "10px 40px 10px 36px", border: errors.currentPassword ? "1px solid #ef4444" : "var(--border-color)", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", color: "var(--text-heading)" }} />
        ...
        <div style={{ marginBottom: 10 }}>
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "var(--text-heading)", marginBottom: 6 }}>{t("New Password", { defaultValue: "New Password" })}</label>
          <div style={{ position: "relative" }}>
            <input type={showNew ? "text" : "password"} value={newPassword} onChange={(e) => { setNewPassword(e.target.value); }} placeholder={t("Enter new password", { defaultValue: "Enter new password" })} style={{ width: "100%", padding: "10px 40px 10px 12px", border: "var(--border-color)", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", color: "var(--text-heading)" }} />
        ...
        <div style={{ marginBottom: 24 }}>
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "var(--text-heading)", marginBottom: 6 }}>{t("Confirm New Password", { defaultValue: "Confirm New Password" })}</label>
          <div style={{ position: "relative" }}>
            <input type={showConfirm ? "text" : "password"} value={confirmPassword} onChange={(e) => { setConfirmPassword(e.target.value); setErrors((prev) => ({ ...prev, confirmPassword: "" })); }} placeholder={t("Confirm new password", { defaultValue: "Confirm new password" })} style={{ width: "100%", padding: "10px 40px 10px 12px", border: errors.confirmPassword ? "1px solid #ef4444" : "var(--border-color)", borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", color: "var(--text-heading)" }} />
