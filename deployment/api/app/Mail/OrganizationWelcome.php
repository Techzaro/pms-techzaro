    ...
    private function buildHtml(): string
    {
        ...
        if ($subscription && $subscription->plan) {
            ...
            if ($subscription->plan->slug === 'trial') {
                ...
                $effectiveUsers = $subscription->custom_max_users ?? $subscription->plan->max_users;
                $effectiveProjects = $subscription->custom_max_projects ?? $subscription->plan->max_projects;
                $effectiveStorage = $subscription->custom_max_storage_gb ?? $subscription->plan->max_storage_gb;
                $maxUsers = $effectiveUsers == 9999 ? 'Unlimited' : $effectiveUsers;
                $maxProjects = $effectiveProjects == 9999 ? 'Unlimited' : $effectiveProjects;
                $planLine1 = "Plan: {$planName}";
                $planLine2 = "{$billingPeriod} billing — {$currency} {$amount}/{$periodShort}";
                $planLine3 = "Limits: {$maxUsers} users, {$maxProjects} projects, {$effectiveStorage} GB storage";
            }
        } else {
