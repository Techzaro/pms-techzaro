    ...
    public function up(): void
    {
        ...
        Schema::table('tasks', function (Blueprint $table) {
            if (!Schema::hasColumn('tasks', 'task_code')) {
                $table->string('task_code', 30)->nullable();
            }
        });

        Schema::table('deliverables', function (Blueprint $table) {
            if (!Schema::hasColumn('deliverables', 'subtask_code')) {
                $table->string('subtask_code', 40)->nullable();
            }
        });
            ...
            if (empty($dlv->subtask_code)) {
                ...
                if ($taskId) {
                    ...
                    DB::table('deliverables')->where('id', $dlv->id)->update([
                        'subtask_code' => ($task ? ($task->task_code ?? 'TSK-'.$taskId) : 'TSK-0') . '.' . $dlvCounters[$taskId],
                    ]);
                } else {
